...
\wninline{Chyba dobrze by było dodać $\eps$ do parametrów struktury danych, i.e. $\mathsf{MappingExample}_{\F, H, k, \eps}$}
...
\begin{lemma} \label{lem:restore-by-sampling}
	...
	Suppose that for any fixed weighting function $w : V(G) \to \Z$, there exists a data structure $\mathsf{WeightedSum}_{f, w}(G)$ that for a dynamic graph $G$ can be initialized and then answer queries by reporting $\sum_{v \in V(G)} f(G, v)w(v)$ after updates to $G$.
	
	Also, suppose that either:
	\begin{enumerate}
		\item \label{it:binary-f} $f(G, v) \in \{0, 1\}$ for all valid arguments, or
		\item \label{it:general-f} there exists a data structure $\mathsf{Verify}_{f}(G)$ that for a dynamic graph $G$ can be initialized and then, for a given vertex $v$, answers queries of checking if $f(G, v)>0$.
	\end{enumerate}
	Then, there exists a randomized data structure $\mathsf{RetrieveVertex}_{f}(G)$ with no false positives that can be initialized and then answer queries by reporting any $v$ such that $f(G, v) > 0$, or that no such vertex exists. If the initialization time, amortized time complexity per a single update, amortized query time, and space complexity of $\mathsf{WeightedSum}_{f, w}(G)$ are $\Oh(I_R)$, $\Oh(T_R)$, $\Oh(Q_R)$ and $\Oh(S_R)$, respectively, and in the case \cref{it:general-f} the initialization time, amortized time complexity per a single update, amortized query time, and space complexity of $\mathsf{Verify}_f(G)$ are $\Oh(I_V)$, $\Oh(T_V)$, $\Oh(Q_V)$ and $\Oh(S_V)$, respectively, then the initialization time, amortized time complexity per a single update, amortized query time, and space complexity of $\mathsf{RetrieveVertex}_f(G)$ are $\Oh((I_R+I_V+|G|) \cdot \log |G| \cdot \log \frac{1}{\eps})$, $\Oh((T_R+T_V) \cdot \log |G| \cdot \log \frac{1}{\eps})$, $\Oh((Q_R+Q_V) \cdot \log |G| \cdot \log \frac{1}{\eps})$ and $\Oh((S_R+S_V+|G|) \cdot \log |G| \cdot \log \frac{1}{\eps})$, respectively, where $\eps$ is the probability of a false negative and where we assume that $I_V = T_V = Q_V = S_V = 0$ in the case \cref{it:binary-f}, where $\mathsf{Verify}$ is not required.
\end{lemma}	
...
\begin{proof}
	...
	Hence, we can set $\mathsf{WeightedSum}_{f, w}(G) \coloneqq \mathsf{WeightedMappings}_{\F, H, w'}(G)$.
