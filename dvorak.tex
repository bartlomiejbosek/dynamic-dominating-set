...
\begin{lemma} \label{lem:restore-by-sampling}
	...
	\begin{enumerate}
		...
		\item \label{it:general-f} there exists a data structure $\mathsf{Verify}_{f}(G)$ that for a dynamic graph $G$ can be initialized and then, for a given vertex $v$, answer queries of checking if $f(G, v)>0$.
	\end{enumerate}
