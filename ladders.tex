...
$r$ - the distance. The bipartite graph $B(\delta^k_r)$ induced
by formula
$\delta^k_r$ is then defined as follows. The left side $L$
consists of all tuples $(v_1,\ldots,v_k) \subseteq V^k$ of
vertices of the input graph $G$. The left side $L$ represents
...
a feasible solution is a neighbour in $B(\delta^k_r)$ to all
vertices $v \in R$. Unfortunately we cannot afford to construct
the entire graph $B(\delta^k_r)$.
Algorithm~\ref{alg:1} searches $B(\delta^k_r)$ by successively
...
semi-ladder in $B(\delta^k_r)$. Thus, the authors define the
...
$\sli(B(\delta^k_r))$. This in turn, as we shall see later,
...
\begin{definition}
The $\nearVertexDS{r,n}$ data structure maintains a dynamic graph $G$ by allowing the following operations:
\begin{itemize}
...
\begin{definition}
The $\farVertexDS{r,n}$ data structure maintains a dynamic graph $G$ by allowing the following operations:
\begin{itemize}
 \item $\init{G}$ - initializes with a graph $G$ on $n$ vertices
 \item $\add{uv}$ - inserts an edge $uv$ to $G$
 \item $\remove{uv}$ - removes an edge $uv$ from $G$
 \item $\far{v}$ - returns a vertex at distance at least $r+1$ from
  $v$ if such vertex exists, otherwise it returns $\bot$.
\end{itemize}
\end{definition}


\begin{theorem}
Using $\nearVertexDS{r,n}$ and $\farVertexDS{r+1,n+1}$ data
structures,
one can implement Algorithm~\ref{alg:1} in time
$\sli(B(\delta^k_r)) \cdot ( k^{\sli(B(\delta^k_r))+1}
\mathsf{Q_{nv}} +
2k \cdot \mathsf{U_{fv}} + \mathsf{Q_{fv}})$, where
$\mathsf{Q_{nv}}$ is the query time
of $\nearVertexDS{r,n}$ and $\mathsf{U_{fv}}$ and
$\mathsf{Q_{fv}}$ is the update
and the query time of
$\farVertexDS{r+1,n+1}$ respectively.
\end{theorem}
\begin{proof}We show how to implement procedures $\candidate{W}$ and $\notDominated{S}$.
\paragraph*{Candidate.}We set up and maintain a
$\nearVertexDS{r,n}$ data structure on the
...
$\farVertexDS{r+1,n+1}$ data
structure on the input dynamic graph $G$ and some auxiliary
vertex $q \notin V(G)$. Each time a query
$\notDominated{S}$ is used by Algorithm~\ref{alg:1}, we issue
$k$ addition operations $\add(q,u)$ for each $u \in S$. We
then issue $\far{q}$ query, which returns a vertex $v$ at
distance at least $r+2$ from $q$. Thus, $v$ is at distance at
least $r+1$ from all the vertices of $S$ in $G$. Thus, $v$ is
a valid witness that $S$ is not a feasible solution, or, in
other words $\delta^k_r(S,v)$ does not hold. After that,
we remove all the previously added edges incident to the
auxiliary vertex $q$, and we return a vertex $v$ if it exists,
or $\bot$ otherwise. This takes $k$ addition and removal operations
and one query operation on $\farVertexDS{r+1,n+1}$ data structure.
