...
the authors introduce a number of tools from logic and combinatorial graph theory.
...
\begin{itemize}
 ...
 \item $\add{uv}$ - inserts an edge $uv$ to $G$
 \item $\remove{uv}$ - removes an edge $uv$ from $G$
 \item $\far{v}$ - returns a vertex at distance at least $r$ from
...
one can implement Algorithm~\ref{alg:1} in time \azinline{some time.}
\end{theorem}
\begin{proof}We show how to implement procedures $\candidate{W}$ and $\notDominated{S}$.
\paragraph*{Candidate.} We implement procedure $\candidate{W}$ as follows. We enumerate all
distance profiles for every vertex in $W$.
\end{proof}
