...
Therefore $\add{xy}$ runs in expected amortized time $\Oh(\poly(d)\log n)$.
...
Again there are amortized $\Oh(\log n)$ updates to $\mathcal{S}_t$, each affecting a set of size at most $4d+1$, so $\remove{xy}$ also runs in expected amortized time $\Oh(\poly(d)\log n)$.
