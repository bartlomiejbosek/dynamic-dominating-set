...
\michalin{The theorems below need fixing details.}

\begin{theorem}[Brodal--Fagerberg orientation data structure,~\cite{BrodalF99}]\label{thm:BF}
	For every $d\in \N$, there is a dynamic data structure that for a fully dynamic $n$-vertex graph $G$ whose degeneracy is always bounded by $d$, maintains an orientation $\vec{G}$ with maximum out-degree bounded by $4d$. Every update (edge insertion or deletion) can be executed in amortized time $\Oh(TODO)$ and triggers $\Oh(TODO)$ reorientations in $\vec{G}$ in the amortized sense. The data structure can be initialized in time $\Oh(n)$ on an edgeless $n$-vertex graph, and uses $\Oh(n+m)$ words of memory, where $m$ is the edge count of $G$. TODO: reporting component.
\end{theorem}

\begin{theorem}[Assadi--Solomon maximal packing data structure,~\cite{AssadiSolomon21}]\label{thm:AS}
	For every $\Delta\in \N$, there is a randomized dynamic data structure that for a fully dynamic set system $\mathcal{F}$ over an $n$-vertex universe $U$, whose arity is always bounded by $\Delta$, maintains a maximal packing $\mathcal{M}$ in $\mathcal{F}$. TODO: time/space complexity bounds, adversary.
\end{theorem}
...
\item Set system $\mathcal{S}_t\coloneqq\setof{S_t(v)}{v\in V}$ on the universe $V$, stored explicitly. Every set $S_t(v)\in \mathcal{S}_t$ is labelled with a permanent label $v$. For every $v\in V$, we store the current members of $S_t(v)$ explicitly, and also we store the size of $S_t(v)$. Wquivalently, we store the permanent element $v$ together with the current out-neighborhood $N^+_{\vec{G}_t}(v)$ and the cardinality of $\{v\}\cup N^+_{\vec{G}_t}(v)$.
...
\michalin{The paragraph below should be removed from here and moved to the statement of \cref{thm:AS}.}

The packing component supports insertions and deletions of vertices inside labelled hyperedges of a dynamic hypergraph of rank at most $q$.
...
\item Read the newly oriented copy of $xy$ and the list of edges reoriented in during this update.
\item For every chaged oriented edge (inserted, removed, or reoriented), relay the modification to appropriately update $\mathcal{S}_t$ and the packing component, as described in the Synchronization paragraph above.
\end{enumerate}
...
The packing component stores a set system of arity at most $4d+1$ and cardinality $n$, so by \cref{thm:AS}, it uses $\Oh(dn)$ words.
The dominating set returned by $\mathtt{QueryDominatingSet()}$ is not stored explicitly, so it contributes no additional space.
