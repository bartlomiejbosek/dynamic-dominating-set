...
For the persistent space bound, the orientation $\vec G$ and the (out)neighbor dictionaries use $\Oh(nd)$ space because every $d$-degenerate graph on $V$ has at most $d(n-1)$ edges.
...
\begin{theorem}\label{thm:FarVertex-new-deg}
	...
	The amortized time complexity of updates and queries is $\Oh(2^{4d+|S|}\cdot d^2|S|^3\cdot \log^3 n \log \tfrac{1}{\eps})$. The data structure can be initialized for an edgeless $G$ in time $\Oh(n\log n\log \tfrac{1}{\eps})$, and uses $\Oh(2^{4d}\cdot dn\log n\log \tfrac{1}{\eps})$ space at all times.
\end{theorem}
\begin{proof}
