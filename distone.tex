...
\begin{theorem}\label{thm:NearVertex-new-deg}
	...
	\begin{itemize}
		\item $\near{S,1}$: For a given set of vertices $S\subseteq V(G)$, return a vertex $v$ such that $S\subseteq N[v]$, or $\bot$ if no such vertex exists, as well as the value $\sum_{v\colon S\subseteq N[v]} \wei(v)$.
	\end{itemize}
...
\begin{theorem}\label{thm:FarVertex-new-deg}
	...
	The amortized time complexity of updates and queries is $\Oh(2^{4d+|S|}\cdot d^2|S|^3\cdot \log^3 n \log \tfrac{1}{\eps})$. The data structure can be initialized for an edgeless $G$ in time $\Oh(n\log n\log \tfrac{1}{\eps})$, and it uses $\Oh(2^{4d}\cdot dn\log n\log \tfrac{1}{\eps})$ space at all times.
\end{theorem}
\begin{proof}
