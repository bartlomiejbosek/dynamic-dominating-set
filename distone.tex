...
Buckets reduce the query ``given $Z$, find $u$ such that $Z\subseteq N^+(u)$'' into a single dictionary lookup: instead of scanning through all the vertices of $V$, we consult the single list $B(Z)\coloneqq \{u\in V^+ \mid Z\subseteq N^+(u)\}$, maintained in the dictionary.

In the toolkit, we explicitly maintain the following objects at all times.
\begin{itemize}
...
\item The bucket family $B(Z)\coloneqq \{u\in V^+ : Z\subseteq N^+(u)\}$ for all sets $Z\subseteq V$ with $B(Z)$ being nonempty (note that this implies $|Z|\leq \Delta$), stored as linked lists inside a dictionary keyed by the sorted representation of $Z$. Observe that every vertex $u\in V$ is on $2^{|N^+(u)|}\leq 2^\Delta$, hence the total sum of the lengths of the lists stored for the buckets $B(Z)$ is bounded by $2^\Delta\cdot n$. In particular, the dictionary stores at most this many entries.
...
Consequently, one bucket lookup, insertion, or deletion by key costs $\Oh(\log (2^\Delta n))=\Oh(\Delta+\log n)$, while after the bucket is found, accessing the first element of a nonempty bucket costs $\Oh(1)$, checking whether a vertex belongs to $V^+$ costs $\Oh(1)$, and reading $|B(Z)|$ or $|V^+|$ costs $\Oh(1)$.
...
One instance of the toolkit described in this subsection can be initialized on the edgeless graph on $V$ in $\Oh(1)$ time.
It uses $\Oh(nd\cdot 2^{4d})$ space.
An edge insertion or deletion updates the toolkit in $\Oh(2^{4d}\cdot d^2\cdot\log^2 n)$ amortized time and outputs the complete log $(L_{\mathrm{in}}^+,L_{\mathrm{out}}^+,\Lambda^+,\Lambda^-)$.
...
Therefore the amortized number of vertices for which either the set $N^+(u)$ or the predicate $u\in V^+$ may change is $\Oh(\log n)$.
...
Because buckets are defined only for vertices in $V^+$, the old registrations of $u$ form the family $\mathcal{R}_{\mathrm{old}}(u)\coloneqq \{Z\mid Z\subseteq A_{\mathrm{old}}\}$ assuming $u\in V^+_{\mathrm{old}}$, while the new registrations form the family  $\mathcal{R}_{\mathrm{new}}(u)\coloneqq \{Z\mid Z\subseteq A_{\mathrm{new}}\}$ assuming $u\in V^+_{\mathrm{new}}$.
...
Producing one subset key and performing the corresponding bucket lookup, insertion, or deletion costs $\Oh(\Delta+\log n)$, because the key has size at most $\Delta$.
...
Recording these events costs $\Oh(1)$ per registration, so the whole work caused by the vertex $u$ is $\Oh((|\mathcal{R}_{\mathrm{old}}(u)|+|\mathcal{R}_{\mathrm{new}}(u)|)\cdot (\Delta+\log n))=\Oh(2^\Delta\cdot (\Delta+\log n))$ and gives rise to $\Oh(2^\Delta\cdot \Delta)$ records in the log.
...
Since $\Delta=4d$, the whole toolkit uses $\Oh(nd\cdot 2^{4d})$ words of space
\end{proof}
...
\end{align*}
Note that $|U_1(S)|\leq \Delta|S|$, because $|N^+(s)|\leq \Delta$ for each $s\in S$.
...
Suppose $S\subseteq V$ is nonempty.
If $|S|\le \Delta$, then $U_2(S)=B(S)$.
And if $|S|>\Delta$, then $U_2(S)=\emptyset$.
\end{lemma}
\begin{proof}
...
And if $v\in U_2(S)$, then \cref{lem:distone-U2-bucket} yields that $|S|\leq \Delta$ and $v\in B(S)$.
\end{proof}
