...
\paragraph{Procedure \texorpdfstring{$\far{S}$ for $\farVertexDS{1,n}$}{farVertex(S) for FarVertex[1,n]}.}
\begin{enumerate}
...
Thus the only witnesses not represented by $f$ are isolated vertices, and these are handled by the private toolkit stored directly in $\farVertexDS{1,n}$.
