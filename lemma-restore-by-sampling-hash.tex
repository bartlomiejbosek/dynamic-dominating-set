...
and fix any such prime $p$ at initialisation.
...
$(i,\ell)$.
By the convention of the lemma statement, the bounds $I_R,T_R,Q_R,S_R$ for
...
adversary, the probability that the data structure reports failure despite
$X\neq\emptyset$ is at most $\eps$. This is exactly the false-negative
guarantee used in the lemma statement.
...
\emph{Initialization time.}
Choosing a prime $p$ with $8n<p<16n$, sampling the $\lambda$ hash functions,
and fixing the resulting implicit family $\{S_{i,j}\}$ require only one-time
work.
Because the construction is hash-based, the actual representation of this
random family is more economical than explicitly materialising all sets
$S_{i,j}$; nevertheless, as a safe upper bound consistent with the statement,
all one-time work related to choosing $p$ and describing the random family is
at most $O\!\bigl(|G|(L+1)\lambda\bigr)$.
...
Hence the total initialization time is
\[
O\!\bigl((L+1)\lambda\cdot I_R + I_V + |G|(L+1)\lambda\bigr)
=
O\!\left((I_R+I_V+|G|)\cdot \log|G|\cdot
...
\log\tfrac{1}{\eps}\right),
\]
exactly as claimed in the statement.
...
The hash seeds for $h_1,\dots,h_\lambda$ occupy only $O(\lambda)$ machine
words, so the actual hash-based representation of the random family may use
less space than the statement claims.
However, since $|G|\ge 1$ and $(L+1)\lambda\ge 1$, this actual overhead is
certainly bounded from above by $O\!\bigl(|G|(L+1)\lambda\bigr)$.
Therefore, although the implementation does \emph{not} store all sets
$S_{i,j}$ explicitly, the total space is also bounded by
\[
O\!\bigl(S_R(L+1)\lambda + S_V + |G|(L+1)\lambda\bigr)
=
O\!\left((S_R+S_V+|G|)\cdot \log|G|\cdot
\log\tfrac{1}{\eps}\right),
\]
which matches the statement exactly.

In the case \cref{it:binary-f} we adopt the convention
$I_V=T_V=Q_V=S_V=0$, exactly as in the statement.
\end{proof}
